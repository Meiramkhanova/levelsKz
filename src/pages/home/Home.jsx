...
import {
  getFiveSpecializations,
  getSalariesBySpecialization,
  getCompaniesBySpecialization,
} from "../../api/api";
...
const Home = () => {
  ...
  const updateSalaryList = async () => {
    try {
      setLoading(true);
      const response = await getSalariesBySpecialization(
        token,
        chosenSpecialization.name
      );
  ...
  const updateCompanyList = async () => {
    try {
      setLoading(true);
      const response = await getCompaniesBySpecialization(
        token,
        chosenSpecialization.name
      );
  ...
  const toggleCompanySelection = (company) => {
    setSelectedCompanies((prevCompanies) => {
      const index = prevCompanies.findIndex(
        (prevCompany) => prevCompany.companyName === company.companyName
      );
      if (index !== -1) {
        return [
      ...
      <Grid container spacing={2} sx={{ marginTop: "40px" }}>
        {specializations.map((specialization, index) => (
          <Grid item xs={2} md={2} key={index}>
            <Button onClick={() => handleSpecializationSelect(specialization)}>
              <Typography
                sx={{
                  fontWeight: "600",
                  color:
